...
const buildBlackholeOverviewFallback = () => ({
  venues,
  goodPeople: fallbackGoodPeople,
  goodPeopleRows: buildRows(fallbackGoodPeople)
})
...
async function fetchVenueDetail(id) {
  try {
    const data = await callApi('venues.detail', { id })
    return {
      ...data,
      venueTopics: (data.venueTopics || []).map(enrichTopicParticipants),
      mode: 'cloud'
    }
  } catch (error) {
    return {
      ...buildVenueDetailFallback(id),
      mode: 'mock',
      error
    }
  }
...
async function fetchBlackholeOverview() {
  try {
    const data = await callApi('blackhole.overview')
    return {
      ...data,
      mode: 'cloud'
    }
    ...
    return {
      ...buildBlackholeOverviewFallback(),
      mode: 'mock',
      error
    }
  }
...
async function submitVenueRecommendation(payload = {}) {
  try {
    const data = await callApi('venues.recommend', payload)
    return {
      ...data,
      mode: 'cloud'
    }
...
async function rsvpEvent(payload = {}) {
  try {
    const data = await callApi('events.rsvp', payload)
    return {
      ...data,
      mode: 'cloud'
    }
...
async function pinEvent(payload = {}) {
  try {
    const data = await callApi('events.pin', payload)
    return {
      ...data,
      mode: 'cloud'
    }
...
async function createMomentFromConversation(id) {
  try {
    const data = await callApi('moments.create', { id })
    return {
      ...data,
      mode: 'cloud'
    }
...
module.exports = {
  ...
  fetchPersonProfile,
  fetchVenueDetail,
  fetchBlackholeOverview,
  submitVenueRecommendation,
